        ...
        trabajos.map(trabajo => {
          return (
            <article className='work-item' key={trabajo.id}>
              <div className='mask'>
                <a className='ahov' type='button' href={trabajo.url} target='_blank'><img src={'/png/'+trabajo.id+'.png'}/></a>
              </div>
              <span>{trabajos.categorias}</span>
              <a type='button' href={trabajo.url} target='_blank'><h2>{trabajo.nombre}</h2></a>
